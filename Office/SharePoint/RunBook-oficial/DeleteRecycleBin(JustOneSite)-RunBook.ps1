...
foreach($site in $SitesURL){
    ...
    Write-Host "$i) Eliminar ficheiros (Recycle bin) SharePoint do site $site"
    $DateToday = Get-Date
    ...
    Write-Host "Site $site DONE!!!" -ForegroundColor Green
    Write-Host ""
...
{    
    ...
    if($CountItemSecondStage -gt 0)
    {
        ...
        Write-Host "Eliminou mais ficheiros do site $site" -ForegroundColor red
    }
    if($CountItemSecondStage -eq 0)
    {
        Write-Host "DOUBLE CHECK (SECOND STAGE) - $site - OK"
    }
}
